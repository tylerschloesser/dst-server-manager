...
Implements `docs/decisions.md` section 13. Owns the cross-cutting verification strategy, the
root-level command contract, and `scripts/lifecycle-test.ts`. Per-package unit-test lists live in
`docs/game-server.md`, `docs/control-plane.md`, `docs/auth.md`, `docs/storage.md`, `docs/web.md`,
`docs/infra.md`; this doc names only the must-haves from decisions 13. Two rules shape everything:
...
| `pnpm e2e` | Playwright against the local app, starting its own servers | exit 0 |
...
| `pnpm lifecycle-test` | `tsx scripts/lifecycle-test.ts` — section 6, **requires** `AWS_PROFILE=admin` | exit 0 and a PASS table |
...
|---|---|---|---|
| `shared` | none | slug validation (`[a-z0-9-]{1,32}`), status enum, state-item schema round trip, the network guard tests | — |
...
| `api` (session) | HKDF with a fixed test secret | HMAC tamper rejected; expiry honoured; **cross-env rejection: a token minted with `env=test` is rejected by a verifier with `env=prod`, and vice versa, even when both use the same raw secret**; cookie name/flags differ per env | `docs/auth.md` |
...
| `api` (reaper) | fake clock, fake EC2 + store | orphan (session/instance mismatch) → terminate; age > 12 h → `desiredWorldId` nulled, no terminate; age > 12 h 10 min → terminate `reaper-max-age`; heartbeat > 10 min **and** instance > 15 min → terminate `reaper-stale`; instance < 15 min old with a stale heartbeat → untouched; reconcile `running` with no live instance → `stopped`; reconcile `starting` only after 3 min; `now` override cannot make the reaper *less* aggressive | `docs/control-plane.md` |
...
| `infra` | `aws-cdk-lib/assertions` `Template` | bucket policy deny with the exact `NotResource` exemptions; both lifecycle rules (`worlds/` 10/30, `inflight/` 3/7); SG = UDP 10998-10999 only; **both** Lambda permissions (`InvokeFunctionUrl` *and* `InvokeFunction`, each with `AWS:SourceArn` = the distribution); launch-template + `RunInstances` tag specs; `CachingDisabled` + `AllViewerExceptHostHeader`; no SSM parameter resources in any template; DNS: only the ACM CNAME and the `dst.ty.ler.dev` A/AAAA aliases | `docs/infra.md` |
| `e2e/` | local API (`env=test`) + Vite, started by Playwright | sign-in, world list, start → starting → running (local fake launcher), countdown visible, stop confirmation, switch confirmation, join info + copy button; phone (390×844) and desktop (1440×900) viewports | `docs/web.md` |

## 3. Playwright with no production backdoor

Mechanism (decisions 9): the local API runs with `env=test` and a **test-only** session secret
(`DST_SESSION_SECRET` from `.env.example`, a committed literal — it is not a secret); Playwright
mints `dst_session` itself. Production derives its key by HKDF with `info` containing `env=prod` and
rejects any token whose `env` differs from its own, so a test token is rejected twice over even if
the test secret leaks. The affordances that exist only locally (dev server, fake EC2 launcher,
test-session endpoint) live in modules that each import and reference the shared constant
`LOCAL_ONLY_MARKER = 'DST_LOCAL_ONLY'` at module scope, so no bundler can tree-shake or rename it
away. That gives the orchestrator exact commands:
...
grep -rl "$(grep DST_SESSION_SECRET .env.example | cut -d= -f2)" packages/infra/cdk.out/ ; echo "exit=$?"  # exit=1
```

Live proof against production, asserted by the lifecycle script (section 4, phase 0): a request to
`$ORIGIN/api/me` carrying a token minted with `env=test` and a random secret returns **401**.
...
`--skip-reaper` (skip phases 7-9; ~50 min instead of ~100), `--timeout-minutes N` (default 150; on
...
`test-lifecycle-a` (`hasCaves=true`, `source=generated`, `idleMinutes=3`) and `test-lifecycle-b`
(`hasCaves=false`, `source=generated`, `idleMinutes=3`). Neither has a save at first run, so the
...
`env=test` token → 401 (section 3). Registry writes for A and B; `GET /api/worlds` lists both.
...
`*/save/client_temp`, `*/save/cached_userid`, or any log file. Extract and assert the password line
...
downloaded `sessions/test-*` log → `0` in each case; the script prints the count, never the value.
...
summary `{ now, evaluated, nulledDesire: [instanceId], terminated: [{instanceId, reason}],
reconciled }` so the result is assertable:
...
in decisions-7 order (orphan → max-age → stale) and the first match wins.
